<script lang="ts">
   ...
   * GitHub brand mark.
   *
   * Lucide removed all brand icons in v1 (brand marks are owned by the brands
   * themselves), so `Github` is no longer exported from `@lucide/svelte`.
   * This component keeps the same call signature as a Lucide icon
   * (`<GithubIcon class="h-5 w-5" />`) so it can be dropped into the same slots.
   */
